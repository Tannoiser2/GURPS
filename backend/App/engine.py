...
def _assign_grid_coords(nodes: dict, ids: list) -> None:
    """Assign (grid_x, grid_y) to each node for the visual mini-map.
    Layout: start(col0) | phase1-middles(col1) | phase2-middles(col2) | objective+exit(col3)
    """
    if not ids:
        return
    start_id = ids[0]
    ...
    phase1_mids = [nid for nid in mid_ids if nodes[nid].phase_gate <= 1]
    phase2_mids = [nid for nid in mid_ids if nodes[nid].phase_gate >= 2]

    max_rows = max(1, len(phase1_mids), len(phase2_mids), len(end_ids))

    def centered_y(group_len: int, row_idx: int) -> int:
        return (max_rows - group_len) // 2 + row_idx

    nodes[start_id].grid_x = 0
    nodes[start_id].grid_y = max_rows // 2

    for i, nid in enumerate(phase1_mids):
        nodes[nid].grid_x = 1
        nodes[nid].grid_y = centered_y(len(phase1_mids), i)

    for i, nid in enumerate(phase2_mids):
        nodes[nid].grid_x = 2
        nodes[nid].grid_y = centered_y(len(phase2_mids), i)

    for i, nid in enumerate(end_ids):
        nodes[nid].grid_x = 3
        nodes[nid].grid_y = centered_y(len(end_ids), i)


def _assign_sector_grid_coords(nodes: dict, ids: list, cols: int = 4, rows: int = 4) -> None:
    """Layout sulla griglia consapevole delle connessioni.
    Posiziona ogni nodo nella cella che minimizza la distanza ai vicini già piazzati,
    in modo che edge che dovrebbero essere "principali" finiscano fra celle adiacenti.
    """
    if not ids:
        return
    ...
    def cell_free(x: int, y: int) -> bool:
        return 0 <= x < cols and 0 <= y < rows and (x, y) not in occupied

    def place(nid: str, x: int, y: int) -> None:
        occupied[(x, y)] = nid
        nodes[nid].grid_x = x
        nodes[nid].grid_y = y
    ...
    # 1. Start node sulla colonna 0, centrato verticalmente
    start_id = ids[0]
    place(start_id, 0, rows // 2)
    placed = {start_id}

    # 2. Scelta del prossimo nodo da piazzare: quello che ha più connessioni con nodi già piazzati
    #    (tie-break: ordine in ids). Lo piazziamo nella cella libera che minimizza la somma
    #    delle distanze ai vicini già piazzati.
    remaining = [nid for nid in ids if nid != start_id]
    while remaining:
        # Ordina per: numero di connessioni a già-piazzati (desc), poi indice originale
        def priority(nid: str) -> tuple[int, int]:
            count = sum(1 for nbr in nodes[nid].connections if nbr in placed)
            return (-count, ids.index(nid))
        remaining.sort(key=priority)
        next_id = remaining.pop(0)

        # Vicini già piazzati di next_id
        connected_placed = [
            (nodes[nbr].grid_x, nodes[nbr].grid_y)
            for nbr in nodes[next_id].connections
            if nbr in placed
        ]

        # Trova la cella libera che minimizza:
        # (a) la massima distanza a un vicino piazzato (priorità: vogliamo edge cheb=1)
        # (b) la somma delle distanze
        # (c) la distanza a una cella già occupata in generale (compattezza)
        best_cell = None
        best_score: tuple[int, int, int] | None = None
        for y in range(rows):
            for x in range(cols):
                if not cell_free(x, y):
                    continue
                if connected_placed:
                    distances = [cheb(x, y, px, py) for px, py in connected_placed]
                    max_d = max(distances)
                    sum_d = sum(distances)
                else:
                    max_d = 0
                    # Compattezza: distanza alla cella occupata più vicina
                    if occupied:
                        sum_d = min(cheb(x, y, ox, oy) for ox, oy in occupied.keys())
                    else:
                        sum_d = 0
                # Tie-break: preferisci celle più "centrali" rispetto alla mappa
                center_d = cheb(x, y, cols // 2, rows // 2)
                score = (max_d, sum_d, center_d)
        ...
        if best_cell is None:
            # Griglia esaurita
            best_cell = (0, 0)
        place(next_id, *best_cell)
        placed.add(next_id)


def _node_description(name: str, kind: str, tags: list[str], environment_type: str) -> str:
    if "morto" in tags:
        return f"Luogo del ritrovamento — ogni elemento è una potenziale prova."
    if "obiettivo" in tags:
        return f"Nodo chiave: qui converge il percorso verso la risoluzione."
    if "uscita" in tags:
        return f"Via d'uscita dall'ambiente."
    if "documenti" in tags:
        return f"Contiene archivi e materiale scritto — informazioni cruciali possono essere nascoste qui."
    if "clue" in tags and kind in ["studio", "biblioteca", "archivio", "ufficio", "laboratorio", "cabina"]:
        return f"Zona ricca di indizi: oggetti personali, documenti, tracce fisiche."
    if "pericolo" in tags:
        return f"Zona pericolosa o sorvegliata — muoversi con cautela."
    if "cura" in tags or kind in ["medico", "infermeria"]:
        return f"Punto medico — permette di recuperare risorse e stabilizzare."
    if "loot" in tags:
        return f"Contiene risorse o materiale utile per la missione."
    return f"{kind.capitalize()} nell'area: {environment_type}."
...
def _fixed_manicomio_map() -> MapState:
    raw_nodes = [
        ("N1", "cappella", "cappella", "Fotografie di te attraversano i decenni. Il confessionale custodisce il primo ricordo.", 1, 3, 0, ["clue", "rito"]),
        ("N2", "archivio cartelle cliniche", "archivio", "Cartelle cliniche falsificate. La verità è sepolta nella documentazione.", 1, 1, 1, ["documenti", "clue"]),
        ("N3", "celle", "celle", "Incisioni, messaggi e frammenti di mente. Attento a chi trovi, o a chi trovi te.", 1, 1, 3, ["clue", "pericolo"]),
        ("N4", "corridoio centrale", "corridoio", "Il cuore del manicomio. Ogni passo può essere osservato.", 1, 3, 3, ["accesso", "pericolo"]),
        ("N5", "salone", "salone", "Tracce di una vita normale. Un'illusione ben costruita.", 1, 5, 3, ["clue"]),
        ("N6", "laboratorio sperimentale", "laboratorio", "Quarantasette soggetti falliti. È custodita qui la verità su Ti Cerca.", 2, 5, 1, ["clue", "pericolo", "obiettivo"]),
        ("N7", "ventilazioni", "ventilazioni", "Percorso alternativo e pericoloso. Il doppio si muove attraverso di esse.", 2, 7, 2, ["pericolo", "alternativo"]),
        ("N8", "giardino - falsa uscita", "giardino", "Il muro lesionato non conduce fuori. Solo a un altro ciclo.", 1, 2, 5, ["uscita", "aperto", "trappola"]),
        ("N9", "ufficio del direttore", "ufficio", "Qui si completa il Protocollo. Fuggi prima che Lui Ti Cerca ti convinca di non esistere.", 3, 5, 5, ["obiettivo", "finale", "clue"]),
    ]
    nodes: dict[str, MapNode] = {}
    for nid, name, kind, description, gate, x, y, tags in raw_nodes:
        nodes[nid] = MapNode(
            id=nid,
            name=name,
            kind=kind,
            ...
            contains_enemy=("pericolo" in tags),
            contains_loot=("loot" in tags),
            is_objective=(nid == "N6"),
            is_final=(nid == "N9"),
            special_event="trap" if "trappola" in tags else None,
            grid_x=x,
            grid_y=y,
        )

    edges: dict[str, MapEdge] = {}
    _connect_map_nodes(nodes, edges, "N4", "N1", "open", "navata centrale")
    _connect_map_nodes(nodes, edges, "N4", "N2", "open", "porta archivio")
    _connect_map_nodes(nodes, edges, "N4", "N3", "open", "corridoio celle")
    _connect_map_nodes(nodes, edges, "N4", "N5", "open", "salone")
    _connect_map_nodes(nodes, edges, "N4", "N8", "trap", "falsa uscita", "Percorso accessibile, ma converge in una trappola.")
    _connect_map_nodes(nodes, edges, "N1", "N6", "locked", "passaggio dietro il confessionale", "Si apre con il primo ricordo.")
    _connect_map_nodes(nodes, edges, "N5", "N9", "locked", "scala direzione", "L'ufficio si sblocca solo dopo aver chiarito la verità su Ti Cerca.")
    _connect_map_nodes(nodes, edges, "N6", "N7", "hidden", "condotto di ventilazione", "Percorso del doppio.", discovered=False)
    _connect_map_nodes(nodes, edges, "N7", "N5", "one_way", "grata arrugginita", "La via riporta al salone, ma non garantisce ritorno.")
    _connect_map_nodes(nodes, edges, "N6", "N9", "locked", "accesso riservato TC-1952", "Richiede la chiave identitaria del Soggetto Primario.")

    for node in nodes.values():
        node.connections = list(dict.fromkeys(node.connections))
    # La mappa fissa ha già coordinate hardcoded — non sovrascriverle.
    nodes["N4"].visited = True

    return MapState(
        map_type="mystery_horror",
        theme="manicomio in rovina",
        nodes=nodes,
        connections_meta=edges,
        current_node_id="N4",
        start_node_id="N4",
        objective_node_id="N6",
        extraction_node_id="N9",
    )


# Node pools specifici per ambiente — sovrascrivono il pool generico di genere
_ENVIRONMENT_NODE_OVERRIDES: dict[str, dict] = {
    "treno di lusso": {
        "start":    [("cabina della vittima", "cabina", ["accesso", "clue", "morto"]),
                     ("corridoio del vagone", "corridoio", ["accesso", "clue"]),
                     ("vagone di prima classe", "vagone", ["accesso"])],
        "middle":   [("vagone ristorante", "ristorante", ["clue", "loot"]),
                     ("corridoio notturno", "corridoio", ["clue", "pericolo"]),
                     ("scompartimento bagagli", "deposito", ["clue", "loot"]),
                     ("cabina del sospettato", "cabina", ["clue"]),
                     ("vagone bar", "lounge", ["clue"]),
                     ("compartimento del personale", "servizi", ["clue"]),
                     ("cabina di prima classe riservata", "cabina", ["clue"])],
        "objective": [("cabina del colpevole", "cabina", ["obiettivo", "clue"]),
                      ("scompartimento segreto", "nascondiglio", ["obiettivo", "clue"]),
                      ("vagone di testa", "locomotore", ["obiettivo", "clue"])],
        "exit":     [("porta di servizio posteriore", "uscita", ["uscita"]),
                     ("vagone di coda", "coda", ["uscita"]),
                     ("passaggio di collegamento", "connettore", ["uscita"])],
    },
    "yacht": {
        "start":    [("ponte superiore", "ponte", ["accesso", "aperto", "clue"]),
                     ("salone principale", "salone", ["accesso", "clue"]),
                     ("cabina ospiti", "cabina", ["accesso"])],
        "middle":   [("sala macchine", "motori", ["pericolo", "clue"]),
                     ("stiva di prua", "stiva", ["loot", "clue"]),
                     ("cabina del comandante", "cabina", ["clue"]),
                     ("sala da pranzo privata", "sala", ["clue"]),
                     ("cabina del proprietario", "cabina", ["clue", "morto"]),
                     ("cabina equipaggio", "cabina", ["clue"])],
        "objective": [("cassaforte di bordo", "cassaforte", ["obiettivo", "clue"]),
                      ("cabina del colpevole", "cabina", ["obiettivo", "clue"]),
                      ("sala radio", "radio", ["obiettivo", "dati"])],
        "exit":     [("scialuppa di salvataggio", "scialuppa", ["uscita"]),
                     ("ponte di prua", "ponte", ["uscita", "aperto"]),
                     ("scala di bordo", "scala", ["uscita"])],
    },
    "isola isolata": {
        "start":    [("porto dell'isola", "porto", ["accesso", "aperto"]),
                     ("villa sul promontorio", "villa", ["accesso", "clue"]),
                     ("spiaggia principale", "spiaggia", ["accesso", "aperto"])],
        "middle":   [("biblioteca della villa", "biblioteca", ["clue", "documenti"]),
                     ("studio del proprietario", "studio", ["clue"]),
                     ("cucina e dispensa", "cucina", ["clue", "loot"]),
                     ("alloggi del personale", "alloggi", ["clue"]),
                     ("rimessa delle barche", "rimessa", ["loot", "clue"]),
                     ("terrazza sul mare", "terrazza", ["aperto", "clue"])],
        "objective": [("camera della vittima", "camera", ["obiettivo", "clue", "morto"]),
                      ("bunker segreto", "bunker", ["obiettivo", "clue"]),
                      ("studio segreto", "studio", ["obiettivo", "clue"])],
        "exit":     [("barca di riserva", "barca", ["uscita"]),
                     ("eliporto privato", "eliporto", ["uscita"]),
                     ("sentiero costiero nascosto", "sentiero", ["uscita", "aperto"])],
    },
    "hotel elegante": {
        "start":    [("hall dell'hotel", "hall", ["accesso", "clue"]),
                     ("corridoio del piano nobile", "corridoio", ["accesso"]),
                     ("bar dell'hotel", "bar", ["accesso", "clue"])],
        "middle":   [("camera della vittima", "camera", ["clue", "morto"]),
                     ("sala conferenze riservata", "sala", ["clue"]),
                     ("cucina e dispensa", "cucina", ["clue", "loot"]),
                     ("camera del personale di servizio", "servizi", ["clue"]),
                     ("archivio e ufficio del direttore", "ufficio", ["documenti", "clue"]),
                     ("lounge panoramica", "lounge", ["aperto", "clue"])],
        "objective": [("suite presidenziale", "suite", ["obiettivo", "clue"]),
                      ("camera del sospettato", "camera", ["obiettivo", "clue"]),
                      ("cantina dell'hotel", "cantina", ["obiettivo", "clue"])],
        "exit":     [("uscita di servizio", "uscita", ["uscita"]),
                     ("parcheggio riservato", "garage", ["uscita"]),
                     ("terrazza sul tetto", "terrazza", ["uscita", "aperto"])],
    },
    "residenza aristocratica": {
        "start":    [("ingresso principale", "ingresso", ["accesso"]),
                     ("sala d'attesa degli ospiti", "sala", ["accesso", "clue"]),
                     ("atrio marmoreo", "atrio", ["accesso", "clue"])],
        "middle":   [("biblioteca privata", "biblioteca", ["documenti", "clue"]),
                     ("studio del padrone di casa", "studio", ["clue"]),
                     ("sala da pranzo formale", "sala", ["clue", "loot"]),
                     ("cucina e dispensa", "cucina", ["clue", "loot"]),
                     ("camera del maggiordomo", "camera", ["clue"]),
                     ("archivio di famiglia", "archivio", ["clue", "documenti"]),
                     ("stanza della vittima", "camera", ["clue", "morto"])],
        "objective": [("sala delle accuse", "sala", ["obiettivo"]),
                      ("camera del segreto di famiglia", "camera", ["obiettivo", "clue"]),
                      ("studio del colpevole", "studio", ["obiettivo", "clue"])],
        "exit":     [("uscita sul parco", "giardino", ["uscita", "aperto"]),
                     ("porta sul retro", "porta", ["uscita"]),
                     ("scuderie", "scuderia", ["uscita"])],
    },
    "villa di campagna": {
        "start":    [("cancello d'ingresso", "cancello", ["accesso", "aperto"]),
                     ("veranda principale", "veranda", ["accesso", "aperto"]),
                     ("ingresso della villa", "ingresso", ["accesso", "clue"])],
        "middle":   [("studio privato", "studio", ["clue"]),
                     ("sala da biliardo", "sala", ["clue"]),
                     ("giardino all'italiana", "giardino", ["aperto", "clue"]),
                     ("cantina delle riserve", "cantina", ["pericolo", "clue"]),
                     ("archivio di famiglia", "archivio", ["clue", "documenti"]),
                     ("rimessa degli attrezzi", "rimessa", ["loot", "clue"]),
                     ("stanza della vittima", "camera", ["clue", "morto"])],
        "objective": [("sala delle accuse", "sala", ["obiettivo"]),
                      ("camera del segreto nascosto", "camera", ["obiettivo", "clue"]),
                      ("studio del colpevole", "studio", ["obiettivo", "clue"])],
        "exit":     [("cancello posteriore", "cancello", ["uscita", "aperto"]),
                     ("strada sterrata", "strada", ["uscita", "aperto"]),
                     ("bosco sul retro", "bosco", ["uscita", "aperto"])],
    },
    "ospedale abbandonato": {
        "start":    [("atrio principale", "atrio", ["accesso", "pericolo"]),
                     ("pronto soccorso", "emergenza", ["accesso", "clue"]),
                     ("parcheggio esterno", "parcheggio", ["accesso", "aperto"])],
        "middle":   [("corsia abbandonata", "corsia", ["clue", "pericolo"]),
                     ("sala operatoria", "chirurgia", ["pericolo", "clue"]),
                     ("archivio pazienti", "archivio", ["documenti", "clue"]),
                     ("magazzino farmaceutico", "magazzino", ["loot", "clue"]),
                     ("cappella ospedaliera", "cappella", ["clue"]),
                     ("seminterrato tecnico", "seminterrato", ["pericolo"])],
        "objective": [("reparto psichiatrico chiuso", "reparto", ["obiettivo", "pericolo", "clue"]),
                      ("sala autopsie", "autopsie", ["obiettivo", "morto", "clue"]),
                      ("ufficio della direzione", "ufficio", ["obiettivo", "clue"])],
        "exit":     [("scala antincendio", "scala", ["uscita"]),
                     ("uscita di emergenza", "uscita", ["uscita"]),
                     ("eliporto sul tetto", "tetto", ["uscita", "aperto"])],
    },
    "manicomio in rovina": {
        "start":    [("cancello d'ingresso", "cancello", ["accesso", "pericolo"]),
                     ("atrio di ricezione", "atrio", ["accesso", "clue"]),
                     ("cortile esterno", "cortile", ["accesso", "aperto"])],
        "middle":   [("corridoio delle celle", "corridoio", ["clue", "pericolo"]),
                     ("stanza dei trattamenti", "trattamenti", ["pericolo", "clue"]),
                     ("archivio cartelle cliniche", "archivio", ["documenti", "clue"]),
                     ("cappella dismessa", "cappella", ["clue"]),
                     ("seminterrato delle celle d'isolamento", "isolamento", ["pericolo", "clue"]),
                     ("laboratorio sperimentale", "laboratorio", ["pericolo", "clue"])],
        "objective": [("ufficio del direttore", "ufficio", ["obiettivo", "clue"]),
                      ("sala shock e contenimento", "sala", ["obiettivo", "pericolo", "clue"]),
                      ("cella sigillata", "cella", ["obiettivo", "clue"])],
        "exit":     [("muro di cinta lesionato", "muro", ["uscita", "aperto"]),
                     ("tunnel di servizio", "tunnel", ["uscita"]),
                     ("uscita di emergenza", "uscita", ["uscita"])],
    },
    "villa isolata": {
        "start":    [("cancello d'ingresso sul vialetto", "cancello", ["accesso", "clue"]),
                     ("veranda frontale", "veranda", ["accesso", "aperto"]),
                     ("atrio", "atrio", ["accesso"])],
        "middle":   [("studio privato", "studio", ["clue"]),
...
def _align_map_to_canon(map_state: "MapState", canon: dict) -> None:
    """Ridenomina (in-place) i nodi 'is_objective' e 'is_final' in base a luoghi citati dal canon,
    in modo che il giocatore veda nomi coerenti con la win_condition. No-op se non trova match utili."""
    if not map_state or not canon:
        return
    locations = _extract_canon_locations(canon)
    if not locations:
        return

    obj_node = map_state.nodes.get(map_state.objective_node_id) if map_state.objective_node_id else None
    final_node = map_state.nodes.get(map_state.extraction_node_id) if map_state.extraction_node_id else None

    if obj_node and locations:
        new_name = locations[0]
        obj_node.name = new_name
        obj_node.description = f"{new_name}: scena chiave per la risoluzione narrativa"

    # Se c'è un secondo luogo distinto, lo applichiamo al nodo finale (estrazione narrativa)
    if final_node and len(locations) > 1 and locations[1].lower() != locations[0].lower():
        new_final_name = locations[1]
        final_node.name = new_final_name
        final_node.description = f"{new_final_name}: punto di chiusura della missione"
...
def infer_narrative_mode(genre: str, mission_type: str) -> str:
    mystery_types = {"indagine", "interrogatorio", "ricostruzione", "scoperta del colpevole"}
    if genre in {"detective_classico", "mystery_horror"} or mission_type in mystery_types:
        return "fixed_mystery"
    return "emergent_mission"
...
def _bible_has_time_pressure(adventure_bible: dict | None) -> bool:
    """Restituisce True se la bibbia indica pressione temporale, False se l'avventura è open-ended.
    Le avventure procedurali (senza bibbia) hanno sempre timer attivo."""
    if adventure_bible is None:
        return True
    # Il campo has_time_pressure è esplicito; se assente, default True (retrocompatibilità)
    return bool(adventure_bible.get("has_time_pressure", True))
...
def _has_scene_focus(state: GameState, current_node: MapNode | None, *needles: str) -> bool:
    haystacks = _scene_focus_terms(state, current_node)
...
def _scene_action_cards(
    state: GameState,
    current_node: MapNode | None,
    archetype: str,
    display_terms: list[str],
    allowed_effect_types: set[str],
    support_effect_types: set[str],
) -> list[dict[str, str]]:
    handles = _scene_resolution_handles(state, current_node, display_terms, allowed_effect_types)
    key_terms = _unique_preserve(display_terms, limit=3)
    stakes = _scene_stakes_text(state, current_node)

    def target_hint() -> str:
        return _join_terms(key_terms[:2], current_node.name if current_node else "la zona")

    scene_problem_text = (state.scene.scene_problem if state.scene and state.scene.scene_problem else "").strip()
    scene_resolution_text = (state.scene.scene_resolution if state.scene and state.scene.scene_resolution else "").strip()
    node_name = current_node.name if current_node else "la zona"
    t = target_hint()

    def _reward(effect: str) -> str:
        if scene_resolution_text:
            verb_map = {
                "investigare": f"scopri {scene_resolution_text.lower().rstrip('.')}",
                "rilevare": f"individui {scene_resolution_text.lower().rstrip('.')}",
                "decifrare": f"comprendi come {scene_resolution_text.lower().rstrip('.')}",
                "forzare": f"riesci a {scene_resolution_text.lower().rstrip('.')}",
                "combattere": f"elimini la resistenza e puoi {scene_resolution_text.lower().rstrip('.')}",
                "infiltrarsi": f"raggiungi {t} e puoi {scene_resolution_text.lower().rstrip('.')}",
                "recuperare": f"recuperi l'elemento chiave e {scene_resolution_text.lower().rstrip('.')}",
                "negoziare": f"ottieni che {scene_resolution_text.lower().rstrip('.')}",
                "difendere": f"guadagni il tempo per {scene_resolution_text.lower().rstrip('.')}",
                "stabilizzare": f"stabilizzi la situazione e puoi {scene_resolution_text.lower().rstrip('.')}",
                "ingannare": f"crei la copertura necessaria per {scene_resolution_text.lower().rstrip('.')}",
                "evocare": f"pieghi la forza locale verso {scene_resolution_text.lower().rstrip('.')}",
            }
            return verb_map.get(effect, scene_resolution_text)
        return f"avanzi verso {t} in {node_name}"

    def _risk(effect: str) -> str:
        if scene_problem_text:
            risk_map = {
                "investigare": f"l'indagine rivela solo rumore — {scene_problem_text.lower().rstrip('.')} si aggrava",
                "rilevare": f"il segnale è falso — {scene_problem_text.lower().rstrip('.')} si consolida",
                "decifrare": f"l'interpretazione è errata — {scene_problem_text.lower().rstrip('.')} si chiude ulteriormente",
                "forzare": f"il tentativo fallisce e {scene_problem_text.lower().rstrip('.')} peggiora",
                "combattere": f"lo scontro attira attenzione e {scene_problem_text.lower().rstrip('.')} diventa impossibile",
                "infiltrarsi": f"vieni individuato — {scene_problem_text.lower().rstrip('.')} si attiva",
                "recuperare": f"l'elemento viene compromesso — {scene_problem_text.lower().rstrip('.')} resta bloccato",
                "negoziare": f"l'interlocutore si chiude — {scene_problem_text.lower().rstrip('.')} resta invariato",
                "difendere": f"la posizione cede — {scene_problem_text.lower().rstrip('.')} si aggrava",
                "stabilizzare": f"il contenimento regge solo parzialmente — {scene_problem_text.lower().rstrip('.')} torna",
                "ingannare": f"l'inganno viene scoperto — {scene_problem_text.lower().rstrip('.')} si irrigidisce",
                "evocare": f"la forza reagisce in modo incontrollato e {scene_problem_text.lower().rstrip('.')} si amplia",
            }
            return risk_map.get(effect, f"{scene_problem_text.lower().rstrip('.')} si aggrava")
        return stakes

    cards: list[dict[str, str]] = []
    preferred_order = _preferred_effect_order(archetype)
    ordered_core = [effect for effect in preferred_order if effect in allowed_effect_types and effect not in {"difendere", "stabilizzare"}]
    ordered_core.extend(
        effect for effect in sorted(allowed_effect_types)
        if effect not in {"difendere", "stabilizzare"} and effect not in ordered_core
    )
    ordered_support = [effect for effect in sorted(support_effect_types) if effect in {"difendere", "stabilizzare", "negoziare"}]
    used_effects: set[str] = set()
    used_prompts: set[str] = set()

    def add_card(effect_type: str, prompt: str, tone: str) -> None:
        clean_prompt = (prompt or _EFFECT_TYPE_LABELS.get(effect_type, "agire sulla scena")).strip()
        if effect_type in used_effects or clean_prompt in used_prompts:
            return
        title = _fiction_action_title(effect_type, clean_prompt, state, current_node)
        if tone == "risk":
            title = f"{title} a rischio"
        cards.append({
            "title": title,
            "prompt": clean_prompt,
            "effect_type": effect_type,
            "reward": _reward(effect_type),
            "risk": _risk(effect_type),
            "tone": tone,
        })
        used_effects.add(effect_type)
        used_prompts.add(clean_prompt)

    focus = _scene_primary_entity(state, current_node) or target_hint()

    for effect_type in ordered_core[:2]:
        prompt = _get_prompt_for_effect(effect_type, handles, used_prompts, focus)
        add_card(effect_type, prompt, "core")

    risky_effect = next((effect for effect in ordered_core if effect in {"forzare", "infiltrarsi", "combattere", "ingannare", "evocare"} and effect not in used_effects), "")
    if risky_effect:
        risky_prompt = _get_prompt_for_effect(risky_effect, handles, used_prompts, focus)
        add_card(risky_effect, risky_prompt, "risk")

    if len(cards) < 3 and ordered_support:
        support_type = ordered_support[0]
        support_prompt = _get_prompt_for_effect(support_type, handles, used_prompts, focus)
        add_card(support_type, support_prompt, "support")

    if len(cards) < 3:
        for effect_type in ordered_core:
            fallback_prompt = _get_prompt_for_effect(effect_type, handles, used_prompts, focus)
            add_card(effect_type, fallback_prompt, "core")
            if len(cards) >= 3:
                break
